...
rubens-ares/         rubens-ares — included as a git submodule
...
2. Builds the `rubens-ares` emulator to WASM from the `rubens-ares/` submodule.
...
### Browser emulator (`rubens-ares/` submodule — [rubens-ares](https://github.com/RubenTipparach/rubens-ares))
...
| Script | What it does |
| --- | --- |
| [rubens-ares/install.bat](rubens-ares/install.bat) | Sets up the emulator's build environment: initializes the ares submodule, installs npm deps under `rubens-ares/web/`, and clones + activates Emscripten SDK into `rubens-ares/emsdk/`. |
| [rubens-ares/build.bat](rubens-ares/build.bat) | Fast rebuild — applies `wasm-patches/`, runs `emmake make wasm32=true` in `build/web-ui/`, copies `ares.js` / `ares.wasm` / `ares.data` into `rubens-ares/web/`. Assumes emsdk is already on PATH. |
| [rubens-ares/build-run.bat](rubens-ares/build-run.bat) | Full build-and-serve. Same as `build.bat` but also auto-activates emsdk, stages the ares source tree into `build/`, and starts `http-server` on port 3000. |
| [rubens-ares/run-experiment.sh](rubens-ares/run-experiment.sh) | Profiling helper. Touches known hot files to force a partial rebuild, runs `build.bat`, redeploys, kills any stale server on :3000, and starts `profile-server.js` for a fixed duration. Takes `<label> [duration_sec]`. |
